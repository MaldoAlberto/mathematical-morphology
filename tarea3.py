...
#función de la erosion y dilatacion para k=1 y k=9 level
def erosion_dilatation():
	...
	if var.get() == 1: 
		...
		kernel2 = cv2.getStructuringElement(cv2.MORPH_RECT,(b,b))
	if var.get() == 2:
		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(b,b))
		kernel2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(b,b))
	if var.get() == 3:
		kernel = cv2.getStructuringElement(cv2.MORPH_CROSS,(b,b))
		kernel2 = cv2.getStructuringElement(cv2.MORPH_CROSS,(b,b))

	imagem = cv2.bitwise_not(image)
	dst = cv2.filter2D(imagem, 0, kernel2)	
	dst2 = cv2.filter2D(image, -1, kernel)

	dst = cv2.bitwise_not(dst)
	cv2.imshow('dilation&k1'+path, dst) 
	cv2.imshow('erosion&k9'+path, dst2) 
	cv2.waitKey(0)
...
hist = Button(root, text="Histogram", command=histogram, state="disabled")
hist.place(x=400, y = 1)

mor1 = Button(root, text="Level Fitres", command=window_erosion, state="disabled")
